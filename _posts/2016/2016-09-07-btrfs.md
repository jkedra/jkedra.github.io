...
which helps with managing snapshosts. It can create an compare snapshosts,
...
snapshot data. More approriate command is command `btrfs filesystem show`: 
...
This removes some incremental changes and usually releases sometimes
significant amount of diskspace.

#### challenges

It happend deleting most of my snapshots is not enough and still 
`btrfs fi show` reported the filsystem being full.
